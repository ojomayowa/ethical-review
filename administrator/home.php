...
<main>
    ...
    <table>
        ...
        <tr class="table100-head">
            ...
            <th class="column5"> Approval Status</th>
            <th class="column4"></th>
        </tr>
        </thead>
        <tbody>
        <?php $i = 1;
        while ($row = mysqli_fetch_array($submitted_requests)) { ?>
            <tr>
                <td> <?php echo $i++; ?></td>
                ...
                <td>
                    ...
                    $id = $row['srid'];
                    $eid = $row['eid'];
                    $query = mysqli_query($connection, "SELECT * FROM approval_request_eao LEFT JOIN experiment_approval_officers ON approval_request_eao.eao_id = experiment_approval_officers.staff_id WHERE approval_request_eao.request_id = '$id'");
                    $count = mysqli_num_rows($query);
                    if($count){
                        while ($a = mysqli_fetch_array($query)) {
                            echo $a['name'] . "<br>";
                        }
                        echo "<a href='view-eao-approval-details.php?experiment_id=$eid&id=$id'>View approval details</a>";
                    }
                    ?>
                ...
                <td>
                    <a href="details.php?experiment_id=<?php echo $row['eid']; ?>">Assign to EAO</a>
                </td>
            </tr>
        <?php } ?>
        </tbody>
